 ...
 * Adapted for ESP_MQTT_Gen_Purpose by Murf <mikael@murf.se>
 * Removed the cli interface, Usefull as general purpose do_ds18b20 changes string instead.
 * Changed around the function names to avoid confusion with built-in Functions....
...
static void ds_write( uint8_t v, int parasitePower );
 ...
 * Parameter: <gpio>
 */
static int ICACHE_FLASH_ATTR ds_get_temp(int gpio,char *temp)
{
	...
	if(dbg==1) INFO("DS18B20: After init.\n" );

	r = ds_search(addr);
	if(r)
	{
		if(dbg==1) INFO("DS18B20: Found Device @ %02x %02x %02x %02x %02x %02x %02x %02x\n",
				addr[0], addr[1], addr[2], addr[3], addr[4], addr[5], 
				addr[6], addr[7] );
		if( crc8( addr, 7 ) != addr[7] )
			INFO( "DS18B20: CRC mismatch, crc=%xd, addr[7]=%xd\n",crc8( addr, 7 ), addr[7] );

		switch( addr[0] )
		{
		case 0x10:
			if(dbg==1) INFO("DS18B20: Device is DS18S20 family.\n" );
			break;

		case 0x28:
			if(dbg==1) INFO("DS18B20: Device is DS18B20 family.\n" );
			break;

		default:
			INFO( "DS18B20: Device is unknown family.\n" );
			return 1;
		}
	}
	else { 
	...
	ds_write(0x44, 1 ); // perform temperature conversion

	os_delay_us( 1000000 );

	if(dbg==1) INFO("DS18B20: Scratchpad: " );
	ds_reset();
	select(addr);
	ds_write(0xbe,0); // read scratchpad
	
	for(i = 0; i < 9; i++ ) {
		data[i] = ds_read();
		if(dbg==1) INFO("%2x ", data[i] );
	}
	if(dbg==1) INFO("\n");
	...
	if(dbg==1) INFO("DS18B20: Temperature: %c%d.%d Celsius\n", SignBit ? '-' : '+',Whole, Fract < 10 ? 0 : Fract);
    os_sprintf(temp,"%c%d.%dC", SignBit ? '-' : '+',Whole, Fract < 10 ? 0 : Fract);
	return r;
}
...
static uint8_t ICACHE_FLASH_ATTR ds_reset(void) {
    ...
    if(dbg==1) INFO("DS18B20: ds_reset\n");
	int r;
	uint8_t retries = 125;
